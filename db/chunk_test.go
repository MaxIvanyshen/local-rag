...
func TestSaveChunk(t *testing.T) {
	...
	docID := "test-doc-1"
	_, err := db.Exec("INSERT INTO document (id, name) VALUES (?, ?)", docID, "test document")
	require.NoError(t, err)
	...
	var count int
	err = db.QueryRow("SELECT COUNT(*) FROM chunks WHERE document_id = ? AND chunk_index = ?", docID, chunkIndex).Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)

	// Verify data was stored correctly
	var retrievedData []byte
	err = db.QueryRow("SELECT data FROM chunks WHERE document_id = ? AND chunk_index = ?", docID, chunkIndex).Scan(&retrievedData)
	require.NoError(t, err)
	assert.Equal(t, data, retrievedData)

	// Verify embedding was inserted (check if rowid exists in chunk_embeddings)
	var embeddingCount int
	err = db.QueryRow("SELECT COUNT(*) FROM chunk_embeddings").Scan(&embeddingCount)
	require.NoError(t, err)
